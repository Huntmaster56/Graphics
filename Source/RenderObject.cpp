...
void solveTangents(Vertex *v, size_t vsize, const unsigned *idxs, size_t isize)
{
	for (int i = 0; i < isize; i += 3)
	{
		...
		for (int j = 0; j < 3; ++j)v[idxs[i + j]].tangent = glm::normalize(T + v[idxs[i + j]].tangent);
	}
...
Texture makeTexture(unsigned w, unsigned h, unsigned c, const void * pixels, bool isFloat )
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, (isFloat? i : f), w, h, 0, f, (isFloat? GL_FLOAT : GL_UNSIGNED_BYTE), pixels);
